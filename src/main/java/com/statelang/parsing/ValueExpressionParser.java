...
public final class ValueExpressionParser {
    ...
    public static final Parser<NumberLiteralValue> numberLiteral = Parse.token(LITERAL_NUMBER)
            .map(token -> new NumberLiteralValue(token, Double.parseDouble(token.text())));

    public static final Parser<BooleanLiteralValue> booleanLiteral = Parse.token(LITERAL_BOOLEAN)
            .map(token -> new BooleanLiteralValue(token, token.text().equals("true")));

    public static final Parser<StringLiteralValue> stringLiteral = Parse.token(LITERAL_STRING)
            .map(token -> new StringLiteralValue(token,
                    token.text().substring(1, token.text().length() - 1)));

    public static final Parser<VariableValueNode> variable = Parse.token(IDENTIFIER)
            .map(VariableValueNode::new);
    ...
    private static Parser<ValueExpressionNode> parseLambda(Parser<ValueExpressionNode> lambdaRecursion) {
        var innerExpression = lambdaRecursion
                .between(Parse.token(OPEN_PARENTHESIS), Parse.token(CLOSE_PARENTHESIS));
        ...
        var primaryTerm = Parse.oneOf(
                ...
                variable,
                innerExpression)
                .recover(Parse.success(InvalidValueNode.instance), Report.Kind.VALUE_EXPRESSION_EXPECTED);

        record TokenOperatorPair(Token token, UnaryOperator operator) {
        }

        var signedTerm = Parse.oneOf(
                Parse.token(OPERATOR_PLUS)
                        .map(token -> new TokenOperatorPair(token, UnaryOperator.PLUS)),
                Parse.token(OPERATOR_MINUS)
                        .map(token -> new TokenOperatorPair(token, UnaryOperator.MINUS)))
                .then(operator -> primaryTerm
                        .map(term -> new UnaryValueExpressionNode(term,
                                operator.value().operator(),
                                operator.value().token()))
                        .cast(ValueExpressionNode.class))
                .or(primaryTerm);

        ChainOperatorApplier<BinaryOperator, ValueExpressionNode> createBinaryExpression = (binaryOperator,
                leftTerm,
                rightTerm) -> new BinaryValueExpressionNode(leftTerm, rightTerm, binaryOperator);
        ...
        var relation = addition.then(left -> Parse.oneOf(
                ...
                .then(operator -> addition
                        .map(right -> new BinaryValueExpressionNode(left.value(), right,
                                operator.value()))
                        .cast(ValueExpressionNode.class))
                .or(Parse.success(left::value)));

        var not = Parse.token(OPERATOR_NOT)
                .then(notToken -> relation
                        .map(right -> (ValueExpressionNode) new UnaryValueExpressionNode(right,
                                UnaryOperator.NOT, notToken.value())))
                .or(relation);
